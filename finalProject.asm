...
;     user's goal is to get a high score or to level 17.
;
; Register Usage:
; R0 - I/O and temp
; R1 - sequence length and comparisons
; R2 - pointer to sequence
; R3 - loop counter and ASCII conversion
; R4 - uppercase mask and temp storage
; R5 - sequence index
; R6 - comparison results and loop condition checks
; R7 - return address and delay counter
;*****************************************************************************
    .ORIG x3000

    ADD R0, R1, R2      ; initialize seed values
    AND R0, R0, #7
    AND R1, R1, #7

    LEA R2, seq         ; sequence pointer
    AND R3, R3, #0
    ADD R3, R3, #-15    ; loop counter
    ADD R3, R3, #-5

GenerateLetters
    STR R0, R2, #0      ; store current value in sequence
    ADD R2, R2, #1      ; move to next position
    ADD R4, R0, R1      ; generate next value
    LD  R5, neg26
    ADD R6, R4, R5
    BRn GenerateModOk   ; if in range skip mod
    ADD R4, R4, R5      ; mod 26
    ADD R6, R4, R5
    BRn GenerateModOk
    ADD R4, R4, R5
...
MenuWait
    GETC                ; wait for player to press 1
    LD  R1, ascii1
    NOT R1, R1
    ADD R1, R1, #1
    ADD R2, R0, R1
...
DisplayClearLoop
    ...
    LD  R0, firstround
    BRz DisplayHeader
    LEA R0, correctmsg  ; shows success message after clear
    PUTS
...
DisplayHeader
    ...
    ST  R0, firstround
    LEA R0, remembermsg
    PUTS                ; shows the sequence to memorize
...
DisplayLoop
    ...
    ADD R6, R6, #1
    ADD R6, R5, R6
    BRn DisplayLoop     ; loops through entire sequence

    LD  R0, newline
    OUT

    ST  R7, savedR7     ; virtual timer for player to memorize sequence
    LD  R7, delayouter
...
DisplayDelayInner
    ...
    LD  R3, neg30       ; "timer" ended, sequence hidden
...
;****************************************************************************
Input
    LD  R4, uppermask   ; mask for uppercase letters
    AND R5, R5, #0
    LD  R1, len

InputLoop
    IN                  ; reads key pressed
    AND R0, R0, R4      ; forces uppercase
    LD  R3, neg65
    ADD R0, R0, R3      ; converts from ASCII to integer (0-25)

    LEA R2, seq
    ADD R2, R2, R5
    LDR R3, R2, #0      ; expected letter

    NOT R3, R3
    ADD R3, R3, #1
    ADD R3, R0, R3
    BRnp Wrong          ; player chose wrong

    ADD R5, R5, #1
    NOT R6, R1
    ADD R6, R6, #1
    ADD R6, R5, R6
    BRn InputLoop       ; continue checking sequence

    LD  R3, neg30       ; clear screen once all correct
InputPostScroll
    LD  R0, newline
    OUT
    ADD R3, R3, #1
    ...
    LD  R1, len         ; sequence length increases, difficulty increases
    ADD R1, R1, #1
    ST  R1, len
    ADD R1, R1, #-15
    ADD R1, R1, #-5
    BRz Win             ; player won all levels
...
WrongScrollLoop
    ...
    LEA R0, failmsg
    PUTS                ; displays loss message
    LEA R0, revealmsg
    PUTS                ; ready to show correct sequence
...
WrongRevealLoop
    ...
    LD  R3, pos65
    ADD R0, R0, R3
    OUT                 ; prints out correct sequence
    LD  R0, pos32
    OUT
    ADD R5, R5, #1
    NOT R6, R1
